...
# ====================================================
locals {
  app_minimal_template = file("${path.module}/../policies/app_minimal.json")

  app_minimal_rendered = replace(
    replace(
      replace(
        local.app_minimal_template,
...
resource "aws_iam_policy" "app_minimal" {
  ...
  policy      = local.app_minimal_rendered
}
